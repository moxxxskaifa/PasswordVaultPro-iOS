...
fun MainScreen() {
    ...
    Scaffold(topBar = { CenterAlignedTopAppBar(title = { Text("Password Vault", fontWeight = FontWeight.Bold) }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).padding(20.dp)) {
            ...
            if (saved.isEmpty()) {
                Text("No saved passwords", color = MaterialTheme.colorScheme.MaterialTheme.colorScheme.oncolorScheme.surfaceVariant, fontSize = 14.sp)
            } else {
                        ...
                        Card(shape = RoundedCornerShape(12.dp)) {
                            Row(Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                                Column(Modifier.weight(1f)) {
                                    ...
                                    Text(item.password, fontSize = 12.sp, color = MaterialTheme.colorScheme.MaterialTheme.colorScheme.oncolorScheme.surfaceVariant)
                                }
